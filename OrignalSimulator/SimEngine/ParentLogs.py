...
class ParentLogs(object):
    ...
    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(ParentLogs, cls).__new__(cls)
    ...
    def __init__(self, failIfNotInit=False):
        ...
        if failIfNotInit and not self._init:
            raise EnvironmentError('ParentLogs singleton not initialized.')
        ...
        # open log file
        self.log_output_files = open(self.settings.getOutputFiles(), 'w')
        ...
        self.log_output_files.write(json_string + '\n')

    def logs(self, parentlogs, content):
        """
        :param dict simlog:
        :param dict content:
        """
        ...
        if (self.log_filters != 'all') and (parentlogs['type'] not in self.log_filters):
            return

        # if a key is passed but is not listed in the log definition, raise error
        if ("keys" in parentlogs) and (sorted(parentlogs["keys"]) != sorted(content.keys())):
            raise Exception(
                "Wrong keys passed to log() function for type {0}!\n    - expected {1}\n    - got      {2}".format(
                    parentlogs['type'],
                    sorted(parentlogs["keys"]),
                    sorted(content.keys()),
                )
            )
    ...
    def flush(self):
        ...
        assert not self.log_output_files.closed
        self.log_output_files.flush()
    ...
    def destroy(self):
        # close log file
        if not self.log_output_files.closed:
            self.log_output_files.close()
