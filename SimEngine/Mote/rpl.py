...
class Rpl(object):
    ...
    def indicate_tx(self, cell, dstMac, isACKed):
        ...
        print("CELLLLL::: "+str(cell))
        print("UPDATINGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG ETX")
...
class RplOF0(object):
    ...
    def update_etx(self, cell, mac_addr, isACKed):
        neighbor = self._find_neighbor(mac_addr)
        print("CELLLLLL INFO:: "+str(cell))
        
        print("MAC ADDR:: "+str(mac_addr))
        print("ISACKED:: "+str(isACKed))
        ...
        if neighbor is None:
            ...
            print("NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNUMBER OF TXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
            neighbor['numTx'] += 1
    ...
    def _update_neighbor_rank_increase(self, neighbor):
        print(" CALCULATING ETX : NUMBER OF TRANSMISSIONS = " + str(
            neighbor['numTx']) + " NUMBER OF ACKNOWLEDGEMENTS = " + str(neighbor['numTxAck']))
        ...
        if neighbor['numTxAck'] == 0:
            if neighbor['numTx'] > self.MAX_NUM_OF_CONSECUTIVE_FAILURES_WITHOUT_ACK:
                ...
                print(" ==========================================setting invalid ETX :  " + str(etx))
            else:
                # ETX is not available
                etx = None
                print(" ==========================================ETX is not available :  " + str(etx))
        else:
            etx = float(neighbor['numTx']) / neighbor['numTxAck']
            print(" ==========================================NEW ETX :  " + str(etx))
        ...
        if step_of_rank is None:
            ...
            print("==============================================================RANK INCREASE FOR NEIGHBOUR: " + str(neighbor['rank_increase'])+ "  ETX IS: " + str(etx))
        if neighbor == self.preferred_parent:
            self.rank = self._calculate_rank(self.preferred_parent)
            print("==================================================================FINAL RANK :" + str(
                self.rank) + " FINAL ETX:" + str(etx))
