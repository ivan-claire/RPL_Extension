...
class Rpl(object):
    ...
    def start(self):
        if self.mote.dagRoot:
            ...
            self.of = RplMrHofNone(self)
            self.of.set_rank(d.RPL_MINHOPRANKINCREASE)
            self.of.set_etx(self.ETX_DEFAULT) #### SETTING DEFAULT ETX
    ...
    def _create_DIO(self, dstIp=None):
        ...
        if self.of.rank is None:
            ...
            print("Rank is infinite : "+str(rank))
        else:
            rank = self.of.rank
            print("Rank is : "+str(rank))
            
        if self.of.etx is None:
            etx = self.ETX_DEFAULT
            print("DEFAULT ETX "+str(etx))
        else:
            etx = self.of.etx
            print("Etx is "+str(etx))
        ...
        newDIO = {
            ...
            'app': {
                ...
                'options':       {'type': d.DIO_OPTION_DAGMC, 
                                   'mcs':  {'type':d.DIO_OPTION_DAGMC_ETX,
                                             'etx':  etx
                                            }
                                  }
            },
...
class RplMrHof(Rpl):
    ...
    def update(self, dio):
        ...
        # updating neighbor's rank    
        self._update_neighbor_rank(neighbor, rank)
        
        if etx == self.ETX_DEFAULT: # IF ETX VALUE DOESN'T EXIST, SET TO DEFAULT VALUE
            self.etx = None
    ...
    def get_preferred_parent(self):
        if self.preferred_parent is None:
            ...
            #print("PREFFERED PARENT...."+str(self.preferred_parent['mac_addr']))
    ...
    def update_etx(self, cell, mac_addr, isACKed):
        ...
        if neighbor is None:
            ...
            if isACKed is True:
                neighbor['numTxAck'] += 1
            self._update_neighbor_rank_increase(neighbor) #calls calculate etx
            self._update_preferred_parent()

    def _add_neighbor(self, mac_addr):
        assert self._find_neighbor(mac_addr) is None
        
        print("ADDING NEW NEIGHBOUR...")
        neighbor = {
            'mac_addr': mac_addr,
    ...
    def _calculate_etx(self, neighbor):    
        if neighbor['numTxAck'] == 0:
            ...
            print("==================ETX VALUE====================:"+str(etx)) 
        ############## get metric value from DIO metric and update rank
        if etx is None:
            etx = self.ETX_DEFAULT
    ...
    def _update_neighbor_rank_increase(self, neighbor):
        ...
        #CALCULATE NEW ETX SINCE NEIGHBOR TRANSMISSION VALUES numTx ans numTxACk HAS CHANGED
        new_etx = self._calculate_etx(neighbor)
        self.etx = new_etx
        #use etx to calculate the rank 
        if new_etx > self.UPPER_LIMIT_OF_ACCEPTABLE_ETX:
            step_of_rank = None
            print(str(self.UPPER_LIMIT_OF_ACCEPTABLE_ETX)+":::::::new ETX Greater than acceptable upper etx limit:::::::::::::"+str(new_etx))
        else:
            print(str(self.UPPER_LIMIT_OF_ACCEPTABLE_ETX)+"::::::new ETX LESS than acceptable upper etx limit::::::::::::::::::"+str(new_etx))
            step_of_rank = (3 * new_etx) - 2
        ...
        if neighbor == self.preferred_parent: ###UPDATING RANK AND ETX VALUE THAT WILL BE USED FOR DIO UPDAT
            self.rank = self._calculate_rank(self.preferred_parent)
        print("==================UPDATING RANK IN DIO====================:"+str(self.rank))   
        print("==================UPDATING ETX IN DIO====================:"+str(self.etx))         
        #print("rank to be added :"+str(self.rank)+"rank to be added :"+str(self.etx))
    ...
    def _calculate_rank(self, neighbor):
        if (
            ...
            print("calculated rank:"+str(rank))    
    ...
    def _update_preferred_parent(self):
        ...
        try:
            ...
            ################### calculating new rank for hysteresis
